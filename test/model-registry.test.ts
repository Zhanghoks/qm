...
test("provider-key gating applies only to key-authed harnesses (no over-hiding on CLI-auth harnesses)", () => {
  ...
  assert.deepEqual(modelProviderAvailabilityFor("pi", noKeys, { anthropic: false, openai: true, openrouter: true, deepseek: false }), {
    anthropic: false,
    openai: true,
    openrouter: true,
    deepseek: false,
  ...
  assert.deepEqual(
    modelProviderAvailabilityFor(
      "opencode",
      { anthropic: true, openai: true, openrouter: true, deepseek: false },
  ...
  assert.deepEqual(modelProviderAvailabilityFor("codex", { anthropic: false, openai: true, openrouter: false, deepseek: false }), {
    anthropic: false,
    openai: true,
    openrouter: false,
    deepseek: false,
...
test("web-turn gate refuses a keyless model cleanly, accepts it once the provider is configured", () => {
  ...
  assert.equal(
    validateWebTurnModelOptions({ model: "gpt-5.6-sol" }, null, { anthropic: true, openai: true, openrouter: false, deepseek: false }),
    null,
  );
});
